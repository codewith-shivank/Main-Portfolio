...
export const ConsistencyCheckBadge: React.FC = () => {
  ...
  const [checks, setChecks] = useState<ConsistencyCheckItem[]>([
    {
      ...
      details:
        "Email (codewithshivank@gmail.com) and LinkedIn profile match resume records.",
    },
    {
      item: "Current Employment Records",
      status: "VERIFIED_SYNCED",
      details:
        "Customer Support Associate at Niftel (Swiggy platform) verified against latest resume.",
    },
    {
      item: "Core Technical Stack",
      status: "VERIFIED_SYNCED",
      details:
        "JavaScript, TypeScript, React, Next.js, Node.js, Express, MongoDB, PostgreSQL verified.",
    },
    {
      item: "Project Case Studies",
      status: "VERIFIED_SYNCED",
      details:
        "Portfolio Website and iNoteBook align with resume architecture records.",
    },
  ]);
  ...
  const runConsistencyAudit = async () => {
    setIsVerifying(true);
    try {
      const res = await fetch(apiUrl("/api/consistency-check"));
      if (res.ok) {
    ...
    <>
      ...
      >
        ...
        <span className="font-semibold">
          Resume-Portfolio Consistency: 100% Synced
        </span>
      </button>
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-neutral-500 dark:text-neutral-400">
                    Automated comparison between portfolio state & verified
                    resume.pdf
                  </p>
                </div>
              </div>
